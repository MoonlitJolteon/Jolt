...
module.exports = {
    ...
    async getTeamInfoCache(teamName) {
        ...
        let data = await cache.fetchCustomExpire(`vrml/Teams/teamIDs/${teamName.toLowerCase()}`, this.getTeamID, teamName, ONE_DAY * 7);
        
        if(data[0] == undefined) return "Invalid🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚";
        data = await cache.fetch(`vrml/Teams/${data[0].id}/Info`, this.getTeamInfo, data[0].id)
        return data;
    },
    ...
    async getTeamPlayersCache(teamID) {
        let data = await cache.fetch(`vrml/Teams/${teamID}/Players`, this.getTeamPlayers, teamID);
        return data;
    },
    
    async getPlayerCache(playerName) {
        let ONE_DAY = (24 * (60* (60 * (60 * 1000 /* 1 Second */) /* 1 Minute */) /* 1 hour */) /* 1 Day */);
        let data = await cache.fetchCustomExpire(`vrml/detailedPlayerData/${playerName}`, this.getPlayer, playerName, ONE_DAY);
    ...
    async getPlayerAvatarCache(playerName) {
        let ONE_DAY = (24 * (60* (60 * (60 * 1000 /* 1 Second */) /* 1 Minute */) /* 1 hour */) /* 1 Day */);
        let url = "Invalid🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚🥚";
        let data = await cache.fetchCustomExpire(`vrml/playerData/${playerName}`, this.findPlayer, playerName,  ONE_DAY * 7);
    ...
    async getTeamsCache() {
        let data = await cache.fetch("vrml/teams", this.getTeams)
        return data
    },
    ...
    async getTeamMatchesCache(historic, teamID) {
        let data;
        if(historic) {
            data = cache.fetch(`vrml/Teams/${teamID}/HistoricMatches`, this.getTeamHistoricMatches, teamID);
        } else {
            data = cache.fetch(`vrml/Teams/${teamID}/UpcomingMatches`, this.getTeamUpcomingMatches, teamID);
        }
