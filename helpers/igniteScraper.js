...
const getStats = async (name, db) => {
    ...
    await rp(url).then((html) => {
        ...
        for(let i = 0; i < items.length; i++) {
            ...
            if(statsTmp[0] == undefined) return (data = invalid);
...
module.exports = {
    ...
    async getPlayerStats(playerName) {
        ...
        let data = await fetchCustomExpireTwoArgs(`igniteStats/personal/${playerName}`, getStats, playerName, 'personal', ONE_DAY)
        
        let stats = data == invalid ? invalid : JSON.parse(data);
        
        if(stats != invalid) return stats;

        data = await fetchCustomExpireTwoArgs(`igniteStats/official/${playerName}`, getStats, playerName, 'personal', ONE_DAY)
